...
for i_ts = 0 : (length(nexData.raw.chan_info)-1)
    ...
    nexData.events{(2*linesOn((j)))-1}.timestamps(channelCount((2*linesOn((j)))-1)) = nexData.raw.sample_point(1)-nexData.data.timestamp(2);
    ...
    if ~isempty(linesOff)
       ...
       nexData.events{2*linesOff(k)}.timestamps(channelCount(2*linesOff(k))) = nexData.raw.sample_point(1)-nexData.data.timestamp(2);
    ...
    if ~isempty(linesOn)
    for j=1:length(linesOn)
    nexData.events{(2*linesOn((j)))-1}.timestamps(channelCount((2*linesOn((j)))-1)) = nexData.raw.chan_info(i_ts+1)-nexData.data.timestamp(2);
    channelCount((2*linesOn((j)))-1)= channelCount((2*linesOn((j)))-1) +1;
    end
    end
    ...
    if ~isempty(linesOff)
        for k=1:length(linesOff)
    nexData.events{2*linesOff(k)}.timestamps(channelCount(2*linesOff(k))) = nexData.raw.chan_info(i_ts+1)-nexData.data.timestamp(2);
    channelCount(2*linesOff(k))= channelCount(2*linesOff(k))+1;
        end
    end
end
...
for i_ts = 0 : (length(nexData2.raw.chan_info)-1)
    ...
    if ~isempty(linesOn)
    for j=1:length(linesOn)
    nexData2.events{(2*linesOn((j)))-1}.timestamps(channelCount((2*linesOn((j)))-1)) = nexData2.raw.sample_point(1)-nexData.data.timestamp(2);
    channelCount((2*linesOn((j)))-1)= channelCount((2*linesOn((j)))-1) +1;
    end
    end
    ...
    if ~isempty(linesOff)
        for k=1:length(linesOff)
       nexData2.events{2*linesOff(k)}.timestamps(channelCount(2*linesOff(k))) = nexData2.raw.sample_point(1)-nexData.data.timestamp(2);
    channelCount(2*linesOff(k))= channelCount(2*linesOff(k))+1;
        end
    end
    else
    ...
    if ~isempty(linesOn)
    for j=1:length(linesOn)
    nexData2.events{(2*linesOn((j)))-1}.timestamps(channelCount((2*linesOn((j)))-1)) = nexData2.raw.chan_info(i_ts+1)-nexData.data.timestamp(2);
    channelCount((2*linesOn((j)))-1)= channelCount((2*linesOn((j)))-1) +1;
    end
    end
    ...
    if ~isempty(linesOff)
        for k=1:length(linesOff)
    nexData2.events{2*linesOff(k)}.timestamps(channelCount(2*linesOff(k))) = nexData2.raw.chan_info(i_ts+1)-nexData.data.timestamp(2);
    channelCount(2*linesOff(k))= channelCount(2*linesOff(k))+1;
        end
    end
end
